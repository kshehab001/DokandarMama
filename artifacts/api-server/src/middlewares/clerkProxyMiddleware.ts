 ...
 *
 * Proxies Clerk Frontend API requests through your own domain, so Clerk
 * authentication works on a custom domain without a CNAME record pointing
 * at Clerk's infrastructure.
 *
 * AUTH CONFIGURATION: manage users, enable/disable login providers (Google,
 * GitHub, etc.), and set app branding from your Clerk dashboard at
 * https://dashboard.clerk.com — create a Clerk application there, then copy
 * its publishable/secret keys into this app's environment variables
 * (VITE_CLERK_PUBLISHABLE_KEY for the client, CLERK_SECRET_KEY for the
 * server; see the deployment guide for the full list).
 *
 * IMPORTANT:
